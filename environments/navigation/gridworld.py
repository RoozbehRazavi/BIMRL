...
class GridNavi(gym.Env):
    ...
    def visualise_behaviour(env,
        ...
        episode_returns = []
        episode_lengths = []
        if policy_type_status is None:
            policy_type_status = torch.ones(size=(1, 1), dtype=torch.long, device=device)
        done_process_mdp = torch.ones(size=(1, 1), dtype=torch.long, device=device)
        ...
        if encoder is not None:
            ...
            episode_brim_output1 = [[] for _ in range(num_episodes)]
            episode_brim_output2 = [[] for _ in range(num_episodes)]
            episode_brim_output3 = [[] for _ in range(num_episodes)]
            episode_brim_output4 = [[] for _ in range(num_episodes)]
            episode_brim_output5 = [[] for _ in range(num_episodes)]

            episode_latent_samples = [[] for _ in range(num_episodes)]
            episode_latent_means = [[] for _ in range(num_episodes)]
            episode_latent_logvars = [[] for _ in range(num_episodes)]
        else:
            episode_brim_output1 = None
            episode_brim_output2 = None
            episode_brim_output3 = None
            episode_brim_output4 = None
            episode_brim_output5 = None
            episode_latent_samples = None
            episode_latent_means = None
            episode_latent_logvars = None

        curr_brim_output1 = curr_brim_output2 = curr_brim_output3 = curr_brim_output4 = curr_brim_output5 =curr_latent_sample = curr_latent_mean = curr_latent_logvar = None

        # --- roll out policy ---
        expr_or_expt = [0] * args.max_rollouts_per_task
        env.reset_task()
        ...
        for episode_idx in range(args.max_rollouts_per_task):
            ...
            if encoder is not None:
                ...
                if episode_idx == 0:
                    # reset to prior
                    curr_brim_output1, curr_brim_output2, curr_brim_output3, curr_brim_output4, curr_brim_output5, brim_hidden_state = encoder.prior(1, mode='brim')
                    curr_latent_sample, curr_latent_mean, curr_latent_logvar, task_inf_hidden_state = encoder.prior(1, mode='task_inf')

                    curr_brim_output1 = curr_brim_output1[0].to(device)
                    curr_brim_output2 = curr_brim_output2[0].to(device)
                    curr_brim_output3 = curr_brim_output3[0].to(device)
                    curr_brim_output4 = curr_brim_output4[0].to(device)
                    curr_brim_output5 = curr_brim_output5[0].to(device)
                    curr_latent_sample = curr_latent_sample[0].to(device)
                    curr_latent_mean = curr_latent_mean[0].to(device)
                    curr_latent_logvar = curr_latent_logvar[0].to(device)
                episode_brim_output1[episode_idx].append(curr_brim_output1[0].clone())
                episode_brim_output2[episode_idx].append(curr_brim_output2[0].clone())
                episode_brim_output3[episode_idx].append(curr_brim_output3[0].clone())
                episode_brim_output4[episode_idx].append(curr_brim_output4[0].clone())
                episode_brim_output5[episode_idx].append(curr_brim_output5[0].clone())
            ...
            for step_idx in range(1, env._max_episode_steps + 1):
                ...
                # act
                _, action, _, policy_type_status, _, _ = utl.select_action(args=args,
                                                 expr_policy=expr_policy,
                                                 expt_policy=expt_policy,
                                                 meta_policy=meta_policy,
                                                 state=state.view(-1),
                                                 belief=belief,
                                                 task=task,
                                                 deterministic=True,
                                                 brim_output1=curr_brim_output1.view(-1) if (curr_brim_output1 is not None) else None,
                                                 brim_output2=curr_brim_output2.view(-1) if (curr_brim_output2 is not None) else None,
                                                 ...
                                                 done_process_mdp=done_process_mdp,
                                                 policy_type_status=policy_type_status,
                                                 pretrain=pretrain,
                                                 )
                if step_idx == 1:
                    expr_or_expt[episode_idx] = policy_type_status.item()

                # observe reward and next obs
                [state, belief, task], (rew_raw, rew_normalised), done, infos = utl.env_step(env, action, args)

                for i in range(1):
                    done_process_mdp[i] = infos[i]['done_mdp']

                if encoder is not None:
                    # update task embedding
                    curr_brim_output1, curr_brim_output2, curr_brim_output3, curr_brim_output4, curr_brim_output5, brim_hidden_state,\
                        latent_sample, latent_mean, latent_logvar, task_inf_hidden_state = encoder(
                        action.float().to(device),
                        state,
                        rew_raw.reshape((1, 1)).float().to(device),
                        brim_hidden_state,
                        task_inf_hidden_state,
                        return_prior=False)
        ...
        if encoder is not None:
            episode_brim_output1 = [torch.stack(e) for e in episode_brim_output1]
            episode_brim_output2 = [torch.stack(e) for e in episode_brim_output2]
            episode_brim_output3 = [torch.stack(e) for e in episode_brim_output3]
            episode_brim_output4 = [torch.stack(e) for e in episode_brim_output4]
            episode_brim_output5 = [torch.stack(e) for e in episode_brim_output5]
        ...
        rew_pred_means, rew_pred_vars = plot_bb(env, args, episode_all_obs, episode_goals, reward_decoder,
                                                None, None,
                                                image_folder, iter_idx, episode_beliefs, expr_or_expt, which_policy)

        if reward_decoder:
            plot_rew_reconstruction(env, rew_pred_means, rew_pred_vars, image_folder, iter_idx)

        return episode_brim_output1, episode_brim_output2, episode_brim_output3, episode_brim_output4, episode_brim_output5, episode_latent_means, episode_latent_logvars,\
...
def plot_bb(env, args, episode_all_obs, episode_goals, reward_decoder,
            episode_latent_means, episode_latent_logvars, image_folder, iter_idx, episode_beliefs, expr_or_expt=None, which_policy=0):
    """
    ...
    for episode_idx in range(num_episodes):
        for step_idx in range(num_steps):
            ...
            if episode_latent_means is not None and False:
            ...
            if step_idx == 0:
                if expr_or_expt[episode_idx] == 0:
                    str = 'Episode {} expr'.format(episode_idx + 1)
                else:
                    str = 'Episode {} expt'.format(episode_idx + 1)
                plt.ylabel(str)
    ...
    if image_folder is not None:
        if which_policy == 0:
            plt.savefig('{}/{}expr_behaviour'.format(image_folder, iter_idx))
        elif which_policy == 1:
            plt.savefig('{}/{}expt_behaviour'.format(image_folder, iter_idx))
        else:
            plt.savefig('{}/{}meta_behaviour'.format(image_folder, iter_idx))
        plt.close()
    else:
        plt.show()
