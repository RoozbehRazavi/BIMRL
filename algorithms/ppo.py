        ...
        if policy_optimiser == 'adam':
            ...
            if use_state_encoder and self.args.use_stateful_vision_core:
                ...
                if use_task_inference_latent_encoder:
                    params.extend(self.actor_critic.task_inference_latent_encoder.parameters())
                if use_belief_encoder:
                    params.extend(self.actor_critic.belief_encoder.parameters())
                if use_task_encoder:
                    params.extend(self.actor_critic.task_encoder.parameters())
                if use_rim_level1_output_encoder:
                    params.extend(self.actor_critic.rim_level1_output_encoder.parameters())
                params.extend(self.actor_critic.actor_layers.parameters())
                params.extend(self.actor_critic.critic_layers.parameters())
                params.extend(self.actor_critic.critic_linear.parameters())
