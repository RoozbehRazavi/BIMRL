...
class Hippocampus(nn.Module):
    ...
    def initialise_readout_attention(rim_hidden_state_to_query_layers,
        ...
        for i in range(rim_hidden_state_to_query_layers):
        ...
        curr_dim = value_size
        for i in range(read_memory_to_value_layer):
        ...
        curr_dim = value_size
        for i in range(read_memory_to_key_layer):
    ...
    def prior(self, exploration_batch_size, exploitation_batch_size):
        self.episodic.prior(exploration_batch_size, exploitation_batch_size)

        if self.args.use_hebb:
            self.hebbian.prior(exploration_batch_size, exploitation_batch_size)

    def reset(self, done_episode):
    ...
    def write(self, key, value, rpe, activated_branch):
        ...
        self.episodic.write(memory_key=key_memory, memory_val=value_memory, rpe=rpe, activated_branch=activated_branch)
